...
docker run --rm --name=kubeadm-version wise2ck8s/kubeadm-version:$TRAVIS_BRANCH kubeadm config images list --kubernetes-version=1.12.1 > ${path}/k8s-images-list.txt
...
curl -sSL https://github.com/wise2ck8s/breeze/raw/v1.12/kubernetes-playbook/kubernetes-dashboard-wise2c.yaml.j2 \
    | sed -e "s,k8s.gcr.io,{{ registry_endpoint }}/{{ registry_project }},g" > ${path}/template/kubernetes-dashboard.yml.j2
